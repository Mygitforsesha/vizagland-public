    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        <select
          ...
          className="w-full appearance-none border border-gray-200 rounded-lg px-3 pt-3.5 pb-2.5 text-sm text-primary bg-white outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-colors cursor-pointer"
          aria-label={label}
        >
          {options.map((opt) => (
...
export function BuyRentToggle({
  ...
  return (
    <div
      className={`flex rounded-lg overflow-hidden border border-gray-200 ${compact ? 'text-sm' : ''}`}
      role="group"
